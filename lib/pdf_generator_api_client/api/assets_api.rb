...
module PDFGeneratorAPI
  class AssetsApi
    ...
    # @param generate_qr_code_request [GenerateQRCodeRequest] QR Code configuration
    # @param [Hash] opts the optional parameters
    # @return [GenerateQRCode201Response]
    def generate_qr_code(generate_qr_code_request, opts = {})
      data, _status_code, _headers = generate_qr_code_with_http_info(generate_qr_code_request, opts)
      data
    end
    ...
    # @param generate_qr_code_request [GenerateQRCodeRequest] QR Code configuration
    # @param [Hash] opts the optional parameters
    # @return [Array<(GenerateQRCode201Response, Integer, Hash)>] GenerateQRCode201Response data, response status code and response headers
    def generate_qr_code_with_http_info(generate_qr_code_request, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: AssetsApi.generate_qr_code ...'
      end
      # verify the required parameter 'generate_qr_code_request' is set
      if @api_client.config.client_side_validation && generate_qr_code_request.nil?
        fail ArgumentError, "Missing the required parameter 'generate_qr_code_request' when calling AssetsApi.generate_qr_code"
      end
      # resource path
      local_var_path = '/assets/qrcode'
      ...
      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(generate_qr_code_request)

      # return_type
      return_type = opts[:debug_return_type] || 'GenerateQRCode201Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['JSONWebTokenAuth']

      new_options = opts.merge(
        :operation => :"AssetsApi.generate_qr_code",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: AssetsApi#generate_qr_code\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
