  ...
  class ConvertURL2PDFRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PDFGeneratorAPI::ConvertURL2PDFRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PDFGeneratorAPI::ConvertURL2PDFRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      paper_size_validator = EnumAttributeValidator.new('String', ["a0", "a1", "a2", "a3", "a4", "legal", "tabloid", "letter"])
    ...
    def paper_size=(paper_size)
      validator = EnumAttributeValidator.new('String', ["a0", "a1", "a2", "a3", "a4", "legal", "tabloid", "letter"])
    ...
    def hash
      [url, paper_size, orientation, output, filename].hash
    end
