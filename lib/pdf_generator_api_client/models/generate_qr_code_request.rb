  ...
  class GenerateQRCodeRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PDFGeneratorAPI::GenerateQRCodeRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PDFGeneratorAPI::GenerateQRCodeRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'logo_base64')
        self.logo_base64 = attributes[:'logo_base64']
      end
    ...
    def valid?
      ...
      output_validator = EnumAttributeValidator.new('String', ["file", "base64"])
    ...
    def output=(output)
      validator = EnumAttributeValidator.new('String', ["file", "base64"])
    ...
    def hash
      [content, color, logo_base64, logo_url, output].hash
    end
