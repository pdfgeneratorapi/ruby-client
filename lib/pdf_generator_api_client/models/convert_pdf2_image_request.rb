  ...
  class ConvertPDF2ImageRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PDFGeneratorAPI::ConvertPDF2ImageRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PDFGeneratorAPI::ConvertPDF2ImageRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'file_base64')
        self.file_base64 = attributes[:'file_base64']
      end
    ...
    def list_invalid_properties
      ...
      if !@resolution.nil? && @resolution > 600
        invalid_properties.push('invalid value for "resolution", must be smaller than or equal to 600.')
      end

      if !@resolution.nil? && @resolution < 72
        invalid_properties.push('invalid value for "resolution", must be greater than or equal to 72.')
      end
    ...
    def valid?
      ...
      format_validator = EnumAttributeValidator.new('String', ["png", "jpeg", "jpg"])
      return false unless format_validator.valid?(@format)
      return false if !@quality.nil? && @quality > 100
      return false if !@quality.nil? && @quality < 1
      return false if !@resolution.nil? && @resolution > 600
      return false if !@resolution.nil? && @resolution < 72
    ...
    def format=(format)
      validator = EnumAttributeValidator.new('String', ["png", "jpeg", "jpg"])
    ...
    def resolution=(resolution)
      ...
      if resolution > 600
        fail ArgumentError, 'invalid value for "resolution", must be smaller than or equal to 600.'
      end

      if resolution < 72
        fail ArgumentError, 'invalid value for "resolution", must be greater than or equal to 72.'
      end
    ...
    def hash
      [file_base64, file_url, format, quality, resolution, pages, output, name].hash
    end
