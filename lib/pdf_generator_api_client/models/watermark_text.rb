  ...
  class WatermarkText
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PDFGeneratorAPI::WatermarkText` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PDFGeneratorAPI::WatermarkText`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@size.nil? && @size > 80
        invalid_properties.push('invalid value for "size", must be smaller than or equal to 80.')
      end

      if !@size.nil? && @size < 7
        invalid_properties.push('invalid value for "size", must be greater than or equal to 7.')
      end
      ...
      if !@opacity.nil? && @opacity < 0.1
        invalid_properties.push('invalid value for "opacity", must be greater than or equal to 0.1.')
      end
    ...
    def valid?
      ...
      return false if !@size.nil? && @size > 80
      return false if !@size.nil? && @size < 7
      return false if !@opacity.nil? && @opacity > 1
      return false if !@opacity.nil? && @opacity < 0.1
    ...
    def size=(size)
      ...
      if size > 80
        fail ArgumentError, 'invalid value for "size", must be smaller than or equal to 80.'
      end

      if size < 7
        fail ArgumentError, 'invalid value for "size", must be greater than or equal to 7.'
      end
    ...
    def opacity=(opacity)
      ...
      if opacity < 0.1
        fail ArgumentError, 'invalid value for "opacity", must be greater than or equal to 0.1.'
      end
    ...
    def hash
      [content, color, size, opacity, position, rotation].hash
    end
