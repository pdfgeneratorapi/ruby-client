  ...
  class FormConfiguration
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PDFGeneratorAPI::FormConfiguration` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PDFGeneratorAPI::FormConfiguration`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'outro')
        self.outro = attributes[:'outro']
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@intro.nil? && @intro.to_s.length > 1275
        invalid_properties.push('invalid value for "intro", the character length must be smaller than or equal to 1275.')
      end

      if !@outro.nil? && @outro.to_s.length > 1275
        invalid_properties.push('invalid value for "outro", the character length must be smaller than or equal to 1275.')
      end
    ...
    def valid?
      ...
      return false if !@intro.nil? && @intro.to_s.length > 1275
      return false if !@outro.nil? && @outro.to_s.length > 1275
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] intro Value to be assigned
    def intro=(intro)
      if !intro.nil? && intro.to_s.length > 1275
        fail ArgumentError, 'invalid value for "intro", the character length must be smaller than or equal to 1275.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] outro Value to be assigned
    def outro=(outro)
      if !outro.nil? && outro.to_s.length > 1275
        fail ArgumentError, 'invalid value for "outro", the character length must be smaller than or equal to 1275.'
      end
